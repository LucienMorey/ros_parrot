...
msg = can.Message(
    arbitration_id=msg.frame_id | axisID << 5, is_extended_id=False, data=data
)
...
while True:
    ...
    if msg.arbitration_id == (
        (axisID << 5) | db.get_message_by_name("Heartbeat").frame_id
    ):
...
while True:
    ...
    data = db.encode_message(
        "Set_Input_Pos", {"Input_Pos": setpoint, "Vel_FF": 0.0, "Torque_FF": 0.0}
    )
    msg = can.Message(
        arbitration_id=axisID << 5 | 0x00C, data=data, is_extended_id=False
    )
    bus.send(msg)
    time.sleep(0.01)
